...
Math.asec = function(a) {
    return Math.acos(1 / a)
};
Math.acsc = function(a) {
...
Math.sech = function(a) {
    return 1 / Math.cosh(a)
};
Math.csch = function(a) {
...
Math.asech = function(a) {
    return Math.acosh(1 / a)
};
Math.acsch = function(a) {
...
Math.step = function(a, c, b) {
    return (a < 0 ? (b ? b : 0) : (c ? c : 1))
};
Math.root = function(a, b) {
    return Math.pow(a, 1 / b)
};
Math.factorial = function(b) {
    if (b === 0) {
        return 1
    }
    if (b < 0) {
        ...
        if (b > -1) {
            return Math.factorial(b + 1) / (b + 1)
        }
        return -Math.PI / (Math.factorial(-1 - b) * Math.sin(Math.PI * b))
    }
    var a = b;
    if ((b % 1) === 0) {
        while (b > 1) {
        ...
        a = Math.exp((b + 0.5) * Math.log(b + 1) - b - b * (0.1121105 + b * 0.08106103) / (1.452342 + b * (2.410858 + b)))
    }
    return a
};
Math.Gamma = function(a) {
    return Math.factorial(a - 1)
};
Math.Beta = function(a, b) {
    return Math.Gamma(a) * Math.Gamma(b) / Math.Gamma(a + b)
};
Math.C = function(a, b) {
    var c = 1 / ((a + 1) * Math.Beta(b + 1, a - b + 1));
    return c
};
Math.truncate = function(a, c) {
    var b = c || 0;
    return Math.floor(a * Math.pow(10, b)) / Math.pow(10, b)
};
Math.randomString = function(a, g) {
    var f = (g || "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz").split("");
    var e = [],
        c = Math.random;
    var d = f.length;
    for (var b = 0; b < a; b++) {
...
Math.runif = function(d, c, f) {
    var e = f || 0;
    return Math.truncate((c + Math.pow(10, -e) - d) * Math.random() + d, e)
};
Math.rnormal = function(a, b) {
    a = a || 0;
    b = b || 1;
    var d = 0,
        c = 0;
    ...
    return b * (Math.sqrt(-2 * Math.log(d)) * Math.cos(2 * Math.PI * c)) + a
};
(function() {
    var a = jQuery.fn.attr,
        b = "http://www.w3.org/2000/svg";
    jQuery.fn.attr = function(m, l) {
        var g = this.length;
        if (!g) {
            return this
        }
            ...
            if (c.namespaceURI === b) {
                if (l !== undefined) {
                    ...
                    if ($.isArray(m)) {
                        ...
                        if (typeof m === "object") {
                            for (v in m) {
                                c.setAttribute(v, m[v])
                            }
                        } else {
                            var h = c.getAttribute(m);
                            if (h || h === "0") {
                                h = isNaN(h) ? h : h - 0
                            }
                            return h
...
var MSVG = function($) {
    ...
    if (!document.createElementNS) {
        document.createElementNS = function(link, type) {
        ...
        switch (symbol.ttype) {
            ...
            case UNARY:
                ...
                if (symbol.input == "\\sqrt") {
                    ...
                    if (typeof symbol.acc == "boolean" && symbol.acc) {
    ...
    function setMarkers(obj, options) {
        ...
        for (i = 0; i < lim; i++) {
            ...
            if (type == "-" || markerChars.indexOf(type) < 0) {
                continue
            }
    ...
    function setLabels(p, options) {
        ...
        var dx = (0.5 * size / SVG.xunitlength) / SVG.factor;
        var dy = (0.5 * size / SVG.yunitlength) / SVG.factor;
    ...
    function path(list, options, append) {
        ...
        if (len > 0) {
            ...
            if (options.marker) {
                setMarkers(node, options)
            }
            appendSVG(elmt);
            if (options.label) {
                setLabels(list, options)
            }
        }
    }
    ...
    function arc(options) {
        ...
        appendSVG(elmt);
        if (options.label) {
            setLabels([start, end], options)
        }
    }

    function sector(options) {
        if (!options.center || !options.startangle || !options.endangle || !options.radius) {
            return
        }
        options.sector = true;
        arc(options)
    ...
    function ellipse(center, rx, ry, options) {
        options = options || {};
    ...
    function text(p, st, options) {
        ...
        if (latex) {
            if (p[0] > SVG.xmax || p[0] < SVG.xmin || p[1] > SVG.ymax || p[1] < SVG.ymin) {
                return
            }
    ...
    function list(fun, tmin, tmax, options) {
        ...
        for (t = tmin; t <= tmax; t += inc) {
            xt = g[0](t);
            yt = g[1](t);
            if (isFinite(xt) && isFinite(yt)) {
                pth[pth.length] = [xt, yt]
            }
        }
        return pth
    }
    ...
    function slopeField(fun, options) {
        ...
        for (x = xmin; x <= SVG.xmax; x += dx) {
            for (y = ymin; y <= SVG.ymax; y += dy) {
                gxy = g(x, y);
                if (!isNaN(gxy)) {
